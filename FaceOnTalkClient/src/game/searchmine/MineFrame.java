...
public class MineFrame extends JFrame implements MouseListener,ActionListener {
	//상수
	...
	//constructor
	public MineFrame() {
		super("FaceOnTalk`s search for land mine");
		init();
	}
	...
	public void mouseReleased(MouseEvent event) {
		...
		if (isGamming) {
			...
			if (flagCount == 0) {
				timeThread.setPause();
				isGamming = false;
				new MessageFrame("Clear Stage!!!!",true);				
			}
		}
	}	
	// 0주변 체크 해줌
			...
			if (nNumberList != null) {//주변 숫자들 이미지 바꿔주기
				for (int i = 0; i < nNumberList.size(); i++) {
					...
					if(board[coord.y][coord.x] == 0)
						checkAroundWithZero(coord.y, coord.x);
					else
	...
	private void gameOver() {
		...
		new MessageFrame("Game Over!!!!",false);		
	}
